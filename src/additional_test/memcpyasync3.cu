...
int main(void)
{
  ...
  for (int i = 0; i < loop_size; i++)
  {
    cudaMemcpyAsync(thrust::raw_pointer_cast(device_multivec[i].data()), host_input.data(), size, cudaMemcpyHostToDevice, stream);
  }
  ...
  for (int i = 0; i < loop_size; i++)
  {
    host_multivec[i].resize(vector_size);
  }

  for (int i = 0; i < loop_size; i++)
  {
    cudaMemcpyAsync(host_multivec[i].data(), thrust::raw_pointer_cast(device_multivec[i].data()), size, cudaMemcpyDeviceToHost, stream);
  }
